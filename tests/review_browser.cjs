...
(async()=>{const server=spawn('python',['-m','http.server','8765','--directory','examples/steward_review']);let browser;
try{browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport:{width:1440,height:1000}});const errors=[];page.on('pageerror',e=>errors.push(e.message));
